...
class Transaction {
    ...
    AnalysisRawTransaction(transaction) {
        ...
        //签发者公钥 20字节
        let fromPubkey = transaction.substr(20, 64);
        //gas单价
    ...
    Sign(msg, prikey) {
        let transaction = this.AnalysisRawTransaction(msg);
        let secretKey = Buffer.from(prikey, 'hex');
        let RawTransaction = Buffer.from(msg, 'hex');
        let sigall = nacl.sign(RawTransaction, secretKey);
        let sigHex = this.Bytes2Str(sigall).substring(0, 128);
        let _tra = transaction.version + transaction.type + transaction.nonce + transaction.fromPubkey + transaction.gasPrice + transaction.Amount + sigHex + transaction.toPubkeyHash + transaction.payload;
        let tra = Buffer.from(_tra, 'hex');
        let transha = keccak256(tra);
        let signRawBasicTransaction = transaction.version + transha + transaction.type + transaction.nonce + transaction.fromPubkey + transaction.gasPrice + transaction.Amount + sigHex + transaction.toPubkeyHash + transaction.payload;
        return {
            'txHash': transha,
            'transaction': signRawBasicTransaction
        }
